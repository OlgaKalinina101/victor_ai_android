...
class AudioPlayer(private val context: Context? = null) {
    ...
    /**
     * 🔥 Упрощённый helper для воспроизведения локального файла (кешированный трек)
     * Принимает абсолютный путь к файлу и конвертирует его в file:// URI,
     * далее использует тот же пайплайн, что и playFromUrl().
     */
    fun playFromFile(path: String) {
        val file = File(path)
        if (!file.exists()) {
            Log.e("AudioPlayer", "❌ playFromFile: file does not exist: $path")
            return
        }
        val uri = Uri.fromFile(file)
        Log.d("AudioPlayer", "🎵 playFromFile with local URI: $uri")
        playFromUrl(uri.toString())
    }
    ...
    fun playFromUrl(url: String) {
        try {
            ...
            // 🔍 Явно задаём HTTP DataSource (user-agent + редиректы) — помогает и в дебаге, и в совместимости.
            val httpDataSourceFactory = DefaultHttpDataSource.Factory()
                .setUserAgent("VictorAI-Player")
                .setAllowCrossProtocolRedirects(true)
                .setDefaultRequestProperties(
                    mapOf(
                        "X-VictorAI-Client" to "player"
                    )
                )
    ...
    private val audioFocusChangeListener: AudioManager.OnAudioFocusChangeListener = AudioManager.OnAudioFocusChangeListener { focusChange ->
        ...
        when (focusChange) {
            ...
            AudioManager.AUDIOFOCUS_LOSS -> {
                // Потеряли фокус навсегда (звонок, YouTube, другое приложение)
                // НЕ возобновляем автоматически - пользователь сам нажмет play
                Log.d("AudioPlayer", "🔇 Audio focus LOSS (permanent) - pausing, no auto-resume")

                wasPlayingBeforeFocusLoss = false  // 🔥 НЕ возобновляем автоматически
                pauseInternal()
                abandonAudioFocus()
            }
