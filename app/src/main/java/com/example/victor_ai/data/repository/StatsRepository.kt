...
import com.example.victor_ai.data.network.StatsResponse
...
class StatsRepository(
    ...
    suspend fun syncWithAPI(): Result<LocalStats> = withContext(Dispatchers.IO) {
        try {
            ...
            // 🔥 КРИТИЧНО: Проверяем accountId даже в release (через exception для логирования)
            if (accountId.isBlank()) {
                val errorMsg = "❌ FATAL: accountId пустой!"
                Log.e(TAG, errorMsg)
                throw IllegalStateException(errorMsg)
            }
            if (accountId == "test_user") {
                Log.w(TAG, "⚠️ WARNING: Используется fallback accountId='test_user'")
            }
    ...
    fun shouldSync(): Boolean {
        ...
        val shouldSync = lastUpdate < fiveMinutesAgo
        
        Log.d(TAG, "🕐 Проверка синхронизации:")
        Log.d(TAG, "   lastUpdate: $lastUpdate (${Date(lastUpdate)})")
        Log.d(TAG, "   fiveMinutesAgo: $fiveMinutesAgo (${Date(fiveMinutesAgo)})")
        Log.d(TAG, "   shouldSync: $shouldSync")
